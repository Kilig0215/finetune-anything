...
from datasets.instance_seg import yolo_dataset_collate
...
if __name__ == '__main__':
    ...
    train_adapt_cfg = deepcopy(config.train)
    train_cfg.model.params["adapt"] = False
    train_adapt_cfg.model.params["adapt"] = True
    ...
    train_loader = DataLoader(train_dataset, batch_size=train_cfg.bs, shuffle=True, num_workers=train_cfg.num_workers,
                              drop_last=train_cfg.drop_last, collate_fn=yolo_dataset_collate)
    ...
    val_loader = DataLoader(val_dataset, batch_size=val_cfg.bs, shuffle=False, num_workers=val_cfg.num_workers,
                            drop_last=val_cfg.drop_last, collate_fn=yolo_dataset_collate)
    ...
    model_adapt = get_model(model_name=train_cfg.model.sam_name, **train_adapt_cfg.model.params)
    ...
    runner = get_runner(train_cfg.runner_name)(model, model_adapt, optimizer, losses, train_loader, val_loader, scheduler)
    # train_step
