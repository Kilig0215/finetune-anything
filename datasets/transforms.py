...
class ResizeLongestSideImg(ResizeLongSide):
    ...
    def apply_image(self, image: np.ndarray) -> np.ndarray:
        ...
        """
        (neww, newh), padh = self.get_preprocess_shape(image.size[0], image.size[1], self.target_length)
        image = np.array(resize(image, (newh, neww)))
        image = np.pad(image, ((padh//2, padh//2), (0, 0) , (0, 0)))
        return image
