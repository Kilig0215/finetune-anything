...
class MyDataset_bbox2Seg(Dataset):
    ...
    def get_preprocess_shape(self,oldw: int, oldh: int, long_side_length: int) -> Tuple[int, int]:
        """
        Compute the output size given input size and target long side length.
        """
        scale = long_side_length * 1.0 / max(oldh, oldw)
        neww, newh = oldw * scale, oldh * scale
        neww = int(neww + 0.5)
        newh = int(newh + 0.5)
        if newh < 640:
            newh = 640  # wh padding
        padh = long_side_length-newh
        return (neww, newh), padh
